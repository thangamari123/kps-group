// Company and Corporate Data for KPS Worldwide Logistics Pvt. Ltd.
// EXACT OFFICIAL BROCHURE CONTENT & COMPANY SPECIFICATIONS

export const companyDetails = {
  name: "KPS Worldwide Logistics Pvt. Ltd.",
  shortName: "KPS Worldwide Logistics",
  legacyName: "K.P.S & Co.",
  foundedYear: 1985,
  founder: "Mr. Srinivasa Pragasam (Seenu)",
  managingDirector: "Mr. B.S. Prassanna",
  
  taglines: {
    primary: "Global Supply Chains. Engineered to Scale.",
    about: "Moving Your Commerce. Shaping Global Trade.",
    containerFreight: "Global Reach. Flexible Capacities.",
    customsWarehousing: "Compliant Clearance. Secure Infrastructure.",
    finalCta: "Partner with KPS Worldwide Logistics Today.",
    supportingCta: "Let Our Logistics Engineers Design Your Next Global Movement.",
    heroSub: "Your premier partner for containerized freight, global break bulk, RoRo operations, and complex over dimensional cargo."
  },

  contact: {
    phone: "044 - 45088099",
    email: "info@kpsgroups.net",
    inquiryEmail: "selvamani.c@kpsgroups.net",
    generalEmail: "info@kpsgroups.net",
    website: "www.kpsgroups.net",
    
    // 21. Corporate Office
    corporateOffice: {
      title: "Corporate Office",
      company: "KPS Worldwide Logistics Pvt Ltd.",
      line1: "No. 14, 6th Floor, A.K. Nayak Bhavan",
      line2: "Jahangir Street, Second Line Beach",
      area: "Parrys",
      city: "Chennai",
      pincode: "600001",
      state: "Tamil Nadu",
      country: "India",
      formatted: "No. 14, 6th Floor, A.K. Nayak Bhavan, Jahangir Street, Second Line Beach, Parrys, Chennai 600001, Tamil Nadu, India"
    },
    
    // 22. Regional Office
    regionalOffice: {
      title: "Regional Office",
      building: "Olympia Crystal",
      line1: "No. 1, SIDCO Industrial Estate",
      area: "Guindy",
      city: "Chennai",
      pincode: "600032",
      state: "Tamil Nadu",
      country: "India",
      formatted: "Olympia Crystal, No. 1, SIDCO Industrial Estate, Guindy, Chennai 600032, Tamil Nadu, India"
    },

    // Legacy address reference for backwards compatibility
    address: {
      line1: "No:14, 6th Floor, A.K. Nayak Bhavan",
      line2: "Jahangir Street, Second Line Beach, Parrys,",
      city: "Chennai",
      pincode: "600 001",
      state: "Tamil Nadu",
      country: "India"
    }
  },

  social: {
    facebook: "https://www.facebook.com/prassanna.kps?mibextid=qi2Omg&rdid=2YWQK3s6OhsK9BMw&share_url=https%3A%2F%2Fwww.facebook.com%2Fshare%2F16Xzk83VMY%2F%3Fmibextid%3Dqi2Omg#",
    instagram: "https://www.instagram.com/kps_international_logistics?igsh=MW44d3Mxc3pncmJmZg%3D%3D",
    linkedin: "https://www.linkedin.com/in/bs-prassanna-b09784116/"
  },

  about: {
    title: "ABOUT KPS WORLDWIDE LOGISTICS",
    headline: "Moving Your Commerce. Shaping Global Trade.",
    intro: "At KPS Worldwide Logistics Pvt Ltd., we bridge the gap between standard logistics and complex project engineering.",
    paragraph1: "For four decades, our experienced professionals have planned, executed, and managed freight movements across major international trade lanes, from single LCL cartons to ultra-heavy industrial plants.",
    paragraph2: "We combine logistics expertise, technical planning, regulatory knowledge, and global coordination to deliver reliable cargo movement for both conventional and specialized requirements.",
    detailedDescription: "For four decades, our experienced professionals have planned, executed, and managed freight movements across major international trade lanes, from single LCL cartons to ultra-heavy industrial plants. We combine logistics expertise, technical planning, regulatory knowledge, and global coordination to deliver reliable cargo movement for both conventional and specialized requirements.",
    
    beginningsTitle: "Our Humble Beginnings",
    beginningsText: "Founded in 1985 by Mr. Srinivasa Pragasam, fondly known in the industry as Seenu, KPS has grown into one of the most trusted names in shipping and logistics. With no prior work experience in the field, Seenu started the company at just 24 years old, driven purely by inspiration from his uncle and a deep desire to build something of his own.",
    
    selfMadeTitle: "The Self-Made Journey",
    selfMadeText: "Instead of joining a company or seeking a job, Seenu committed himself to understanding the complexities of the logistics industry. He studied independently, cleared the necessary regulatory examinations, and obtained his operating license — turning ambition into reality.",
    
    encounterTitle: "A Fateful Encounter",
    encounterText: "One of the earliest milestones came through a small act of kindness. During a bank visit, Seenu helped an elderly man with a deposit form. A month later, that gentleman handed his entire logistics business network to Seenu — becoming his first client.",
    
    legacyTitle: "Continuing the Legacy",
    legacyText: "In 2008, after more than two decades of leadership, Mr. Prasssanna B.S., has taken full control of the KPS Groups Operation ensuring the company's core values continue with the next generation while adapting to modern shipping demands."
  },

  // 3. THE KPS ADVANTAGE
  advantagePillars: [
    {
      id: "comprehensive",
      title: "COMPREHENSIVE CAPABILITY",
      desc: "A single-source logistics partner for both standard freight and specialized cargo movements."
    },
    {
      id: "network",
      title: "GLOBAL NETWORK",
      desc: "Seamless multi-port coordination across major international trade lanes and continents."
    },
    {
      id: "engineering",
      title: "ENGINEERING EXCELLENCE",
      desc: "Technical route surveys, cargo planning, lifting plans, and execution support for complex freight."
    },
    {
      id: "regulatory",
      title: "REGULATORY COMMAND",
      desc: "Licensed customs brokerage and documentation expertise to facilitate efficient and compliant cargo clearance."
    }
  ],

  // 19. KPS SERVICE ADVANTAGES
  serviceAdvantages: [
    { title: "Compliant", desc: "Adhering to global trade laws and regulations." },
    { title: "Efficient", desc: "Streamlined processes to reduce clearance time and delays." },
    { title: "Secure", desc: "Safe handling and storage with robust security protocols." },
    { title: "Expertise", desc: "Decades of experience in customs and project logistics." },
    { title: "Global Connect", desc: "Strong network with authorities, partners, and service providers." }
  ],

  founderSection: {
    title: "Founder & Chairman, K.P.S & Co.",
    name: "Mr. Srinivasa Pragasam (Seenu)",
    quote: "Help others without expecting anything in return good things will always follow.",
    text1: "I began this journey in 1985, at the age of 24, without any industry background just inspiration from my uncle and an eagerness to learn. I didn’t join any company or take a job. I simply decided to build something on my own.",
    text2: "With hard work and self-study, I cleared the required exams and obtained my logistics license. One unforgettable moment in my early days came while I was visiting the bank for company work. I noticed an elderly man struggling to fill a cheque deposit form. I helped him just as any young person should. A month later, that same man, now too old to continue in business, handed me his entire logistics operation.",
    experienceBelief: "That experience shaped my belief: Help others without expecting anything in return good things will always follow.",
    teamTitle: "KPS Logistics Team",
    text3: "Since then, we’ve grown K.P.S & Co. with the values of trust, consistency, and long-term partnerships. I’ve always believed in investing in people and building customer relationships that last. In 2008, I proudly handed over the leadership to my son, Mr. Prasssanna B.S., who shares the same commitment and vision for our future."
  },

  mdSection: {
    title: "Managing Director, K.P.S & Co.",
    name: "Mr. B.S. Prassanna",
    text1: "In 2008, at the age of just 20, Mr. B.S. Prassanna stepped into the world of shipping and logistics, taking over the reins of K.P.S & Co. after completing his Bachelor’s degree in Computer Engineering. Though young, he entered the business with a clear vision: to transform the traditional logistics landscape using modern technology and to offer clients more solution-driven, simplified experiences.",
    text2: "Prassanna’s focus from the very beginning has been to reduce complexity and enhance transparency in logistics operations. With a passion for innovation, he identified the key pain points clients faced and began building tech-enabled solutions that not only improved operational efficiency but also strengthened client satisfaction and trust."
  },

  missionVision: {
    missionTitle: "Our Mission",
    mission: "Committed to integrity and service excellence, we empower our employees, strengthen partnerships with our clients and vendors, and harness technology to enhance the well-being of our region and beyond.",
    visionTitle: "Our Vision",
    vision: "Our vision is to create an inclusive and dynamic environment where integrity and service guide our every action. We aim to empower our employees, nurture strong partnerships with our clients and vendors, and leverage the latest technological advancements to drive innovation and growth in our region, ensuring a brighter future for our community."
  },

  values: [
    { id: "trust", title: "Trust", description: "Built over 40+ years in the industry" },
    { id: "innovation", title: "Engineering & Innovation", description: "Adapting to modern global logistics and project engineering challenges" },
    { id: "relationships", title: "Relationships", description: "Personal touch with dedicated long-term client support" },
    { id: "reliability", title: "Reliability & Compliance", description: "100% regulatory compliance and operational excellence" }
  ],

  careers: {
    intro: "At KPS Worldwide Logistics, we believe in moving not just goods, but careers. We are always looking for passionate, reliable, and driven individuals to join our growing team across various departments. As a company committed to efficiency and innovation, we offer a supportive work environment, competitive compensation, and opportunities for growth and development. If you're ready to be part of a dynamic, people-first logistics company, we invite you to apply and take the next step in your career journey with us."
  }
};
